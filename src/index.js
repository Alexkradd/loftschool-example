...
function findAllPSiblings(where) {
    ...
    for (var item of where.children) {
        if (item.nextElementSibling!== null && item.nextElementSibling.matches('p')) {
            el.push(item)
        }
    }
...
function deleteTextNodes(where) {
    for (var item of where.childNodes) {
        if (item.data) {
    ...
    for (var item of root.childNodes) {
        if (item.data) {
            ...
            [...item.classList].forEach(element => {
                if (obj.classes[element]===undefined) {
            ...
            if (obj.tags[item.nodeName]===undefined) {
                obj.tags[item.nodeName]=1
            } else {
                obj.tags[item.nodeName]++
            }
            if (item.children) {
...
function observeChildNodes(where, fn) {
    const listen = new MutationObserver(mutations =>{
        for (var item of mutations) {
            if (item.addedNodes) {
                fn({
                    type: 'insert',
                    nodes: Object.values(item.addedNodes)
                })
            }
            ...
            if (item.removedNodes) {
                fn({
                    type: 'remove',
                    nodes: Object.values(item.removedNodes)
                })
            }
            if (item.target.children) {
                observeChildNodes(item.target, fn)
            }
        }
        
    })

    listen.observe(where, { childList: true })
